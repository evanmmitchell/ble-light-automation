...
One major difficulty that I dealt with in designing the neural network was incorporating previously unseen BLE devices as features dynamically. First, in order to deal with the memory implications of supporting a potentially infinite number of BLE devices, I limited the number of remembered devices to 250. Thus, the MAC addresses of the 250 most recently seen devices are stored and used to format both the features of a sample and the learned weights of the classifier. Because the weights have to be dynamically reformatted, I was limited to a one-layer neural network.
